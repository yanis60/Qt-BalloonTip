...
BalloonTip::BalloonTip( QStyle::StandardPixmap icon, QString title, QString text, int duration, QWidget* parent ) :
  ...
  my_icon = qApp->style()->standardIcon( icon ).pixmap( QSize( 16, 16 ) );
  init();
...
void BalloonTip::paintEvent(QPaintEvent * /*ev*/ ) {
  ...
  path.addPolygon( p );
  path.addRoundedRect( popupRect, 10, 10 );
...
void BalloonTip::setArrowPosition( BalloonTip::ArrowPosition arrowPos ) {
  ...
  my_closeButton->move( r.topRight() - QPoint(5,-5));
}
...
bool BalloonTip::eventFilter(QObject* obj, QEvent* ev) {
  ...
  QPoint pos = QPoint(parentWidget()->pos().x() + 5,qApp->desktop()->height() - parentWidget()->geometry().height()-30);
  qDebug()<<pos;
...
void BalloonTip::setText(QString t)
{
    my_text = t;
    createRects();
}

QString BalloonTip::Message()
{
    return my_text;
}
